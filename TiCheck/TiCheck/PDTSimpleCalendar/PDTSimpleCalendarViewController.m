...
const NSDictionary *___monthMapToChineseDic;

#define cMonthDictionaryGet (___monthMapToChineseDic == nil ? ___monthMapToChineseDic = [[NSDictionary alloc] initWithObjectsAndKeys:\
@"一月", @"JANUARY",\
@"二月", @"FEBRUARY",\
@"三月", @"MARCH",\
@"四月", @"APRIL",\
...
@"十月", @"OCTOBER",\
@"十一月", @"NOVEMBER",\
@"十二月", @"DECEMBER", nil] : ___monthMapToChineseDic)

#define cChineseMonthGet(month) ([cMonthDictionaryGet objectForKey:month])

const NSArray *___ReciteRecordType;

#define cReciteRecordTypeGet (___ReciteRecordType == nil ? ___ReciteRecordType = [[NSArray alloc] initWithObjects:\
@"NewWordTask",\
@"ReviewTask", nil] : ___ReciteRecordType)

#define cReciteRecordTypeString(type) ([cReciteRecordTypeGet objectAtIndex:type])
#define cReciteRecordTypeEnum(string) ([cReciteRecordTypeGet indexOfObject:string])
...
//    [self.collectionView reloadItemsAtIndexPaths:@[ indexPath ]];
    [self updateSelectedDateWithNewDate:newSelectedDate];
...
- (void)updateSelectedDateWithoutDelegateNotify:(NSDate *)newSelectedDate
{
    [self updateSelectedDateWithNewDate:newSelectedDate];
}

- (void)updateSelectedDateWithNewDate:(NSDate *)newSelectedDate
{
    //if newSelectedDate is nil, unselect the current selected cell
    if (!newSelectedDate) {
        [[self cellForItemAtDate:_selectedDate] setSelected:NO];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    if (kind == UICollectionElementKindSectionHeader) {
        ...
        NSString *headerText = [self.headerDateFormatter stringFromDate:[self firstOfMonthForSection:indexPath.section]].uppercaseString;
        NSArray *dateInfo = [headerText componentsSeparatedByCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        headerView.titleLabel.text = [cChineseMonthGet([dateInfo firstObject]) stringByAppendingFormat:@" %@", [dateInfo lastObject]];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    NSString *overlayText = [self.headerDateFormatter stringFromDate:[self firstOfMonthForSection:firstIndexPath.section]].uppercaseString;
    NSArray *dateInfo = [overlayText componentsSeparatedByCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    self.overlayView.text = [cChineseMonthGet([dateInfo firstObject]) stringByAppendingFormat:@" %@", [dateInfo lastObject]];
